...
func newSprint3Fixture(t *testing.T) *sprint3Fixture {
	...
	if dsn == "" {
		t.Skip("requiere SELAVA_TEST_DATABASE_URL para pruebas de PostgreSQL")
	}
	config, err := pgxpool.ParseConfig(dsn)
	mustSprint3(t, err)
	if !strings.HasSuffix(config.ConnConfig.Database, "_test") {
		t.Fatal("la base de pruebas debe terminar en _test")
	}
	...
	t.Cleanup(func() {
		...
		if cleanupErr != nil {
			t.Errorf("limpiando esquema de test: %v", cleanupErr)
		}
		_ = admin.Close(ctx)
	})
	...
	for _, file := range files {
		...
		mustSprint3(t, readErr)
		if _, err = db.Exec(ctx, string(sql)); err != nil {
			t.Fatalf("migracion %s: %v", file, err)
		}
	}
...
func mustSprint3(t *testing.T, err error) {
	t.Helper()
	if err != nil {
		t.Fatal(err)
	}
...
func (f *sprint3Fixture) total(t *testing.T, orderID int, want float64) {
	...
	if math.Abs(total-want) > 0.00001 {
		t.Fatalf("total=%v, esperado=%v", total, want)
	}
}

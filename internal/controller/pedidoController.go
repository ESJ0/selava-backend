...
func (pc *PedidoController) ActualizarEstado(w http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), requestTimeout)
	defer cancel()
	...
	var req models.PedidoEstadoUpdateRequest
	r.Body = http.MaxBytesReader(w, r.Body, maxBodyBytes)
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		respondError(w, http.StatusBadRequest, "cuerpo de la peticion invalido")
		return
	...
	historial, err := pc.service.ActualizarEstado(ctx, pedidoID, &req, claims.UsuarioID)
	if err != nil {
		pc.handleServiceError(w, err)
		return
	}
...
func (pc *PedidoController) ObtenerHistorialEstados(w http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), requestTimeout)
	defer cancel()
	...
	historial, err := pc.service.ObtenerHistorialEstados(ctx, pedidoID)
	if err != nil {
		pc.handleServiceError(w, err)
		return
	}
